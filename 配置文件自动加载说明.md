...
无需修改 JavaScript 代码，只需在 HTML 中添加 `onchange="loadConfig()"` 事件处理器。
...
- ✅ 所有现代浏览器都支持 `onchange` 事件
- ✅ 移动端浏览器完全支持
- ✅ 键盘操作（方向键选择）也会触发 `onchange`
...
```
✅ 已加载配置 - Provider: deepseek, Model: deepseek-coder
```
...
1. **首次加载**：页面加载时会自动加载默认配置或上次选择的配置
2. **切换配置**：每次切换都会立即加载，请确保配置文件有效
3. **网络延迟**：加载配置需要网络请求，可能有短暂延迟
4. **错误处理**：如果配置加载失败，会显示错误信息，不影响其他功能
