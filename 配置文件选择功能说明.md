...
支持通过下拉框选择不同的配置文件，方便在多�?AI 模型或配置之间快速切换�?
...
**默认行为�?*
- 如果没有选择，默认使�?`config.yaml`
- 如果 `config.yaml` 不存在，使用列表中的第一个配�?
...
1. �?`config/` 目录下创建新�?YAML 文件
2. 复制 `config.yaml` 的内容作为模�?
...
```javascript
window.onload = async () => {
    await loadConfigList(); // 1. 先加载配置列�?
    loadConfig();           // 2. 自动加载选中的配�?
    connectLogs();          // 3. 连接日志�?
};
```
...
当用户切换下拉框选项时，`onchange` 事件会自动触�?`loadConfig()` 函数�?
...
- `config/dev.yaml` - 开发环境配�?
- `config/prod.yaml` - 生产环境配置
- `config/test.yaml` - 测试环境配置

### 3. 团队协作

团队成员可以使用各自�?API Key，创建个人配置文件：
- `config/alice.yaml`
- `config/bob.yaml`
