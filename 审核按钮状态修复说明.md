...
审核完成并弹出报告后，"开始审核"按钮仍然显示为"审核中..."状态，没有恢复到可用状态。

## 问题原因

### 1. 递归调用问题

原代码在检测到 `REPORT_URL:` 消息时，会调用 `log('✅ 已在新标签页打开报告')`，导致递归调用 `log()` 函数。
...
按钮状态只在检测到"所有文件审核完成"消息时才恢复，但实际上报告已经打开了。
...
3. **浏览器兼容性**：`window.open()` 可能被浏览器的弹窗拦截器阻止，需要用户允许弹窗。
