...
#### 1.1 配置管理
- ✅ 扩展了 `Config` 结构，添加 `OnlineConfig` 支持
- ✅ 实现了 `SaveConfig` 函数用于保存凭据
- ✅ 支持从配置文件加载SVN凭据

#### 1.2 SVN客户端扩展
- ✅ 新增 `LogEntry` 结构存储提交记录
- ✅ 新增 `NewOnlineClient` 创建在线客户端
- ✅ 实现 `TestConnection` 测试SVN服务器连接
- ✅ 实现 `SearchLog` 搜索SVN提交记录
  - 支持按路径搜索
  - 支持按关键词搜索
  - 支持按作者搜索
  - 支持限制返回数量
- ✅ 实现 `GetRevisionDiff` 获取指定版本的差异
- ✅ 实现 `GetRevisionFiles` 获取指定版本的文件列表
...
#### 2.1 凭据管理
- ✅ CLI模式：使用 `--save` 参数保存到 config.yaml
...
- 提取revision、author、date、message、paths等信息
- 避免引入额外的XML解析库依赖
...
# 使用保存的凭据
svn-reviewer review online --keyword "feature"
```
...
2. **分页功能**：上/下100条功能的UI已实现，但后端逻辑需要完善
...
1. **完整的功能**：支持连接、搜索、选择、审核全流程
2. **双模式支持**：CLI和GUI都可以使用在线模式
3. **用户友好**：凭据记忆、交互式选择、实时反馈
4. **代码复用**：审核逻辑与本地模式完全一致
